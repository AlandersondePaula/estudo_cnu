...
def create_study_schedule(start_date, weeks_data):
    schedule = []
...
def main():
    ...
    if view_option == "📅 Cronograma Geral":
        ...
        # Criar cronograma
        schedule = create_study_schedule(start_date, study_data)
...
#                     # Botão para marcar como concluído
#                     key = f"{resource_type}_{i}_{resource['description'][:20]}"
#                     if st.button(f"✅ Marcar como concluído", key=key):
#                         if 'completed_resources' not in st.session_state:
#                             st.session_state.completed_resources = set()
#                         st.session_state.completed_resources.add(key)
#                         st.success("Recurso marcado como concluído!")
...
#     view_option = st.sidebar.selectbox(
#         "Escolha a visualização:",
#         ["📅 Cronograma Geral", "📖 Estudo por Semana", "📊 Progresso", "🔍 Buscar Recursos"]
#     )
...
#                 st.metric("Total de Semanas", len(schedule))
#             with col2:
#                 total_subjects = sum(len(week['Matérias'].split(', ')) for week in schedule if week['Matérias'])
#                 st.metric("Total de Matérias", total_subjects)
#             with col3:
#                 end_date = datetime.strptime(schedule[-1]['Data Fim'], '%d/%m/%Y')
#                 duration = (end_date - start_date).days
#                 st.metric("Duração (dias)", duration)
...
#         <div style='text-align: center; color: #666;'>
#             📚 Plano de Estudos CNU 2025 - Banca FGV<br>
#             Desenvolvido para otimizar sua preparação para o concurso
#         </div>
#         """,
#         unsafe_allow_html=True
#     )
